...
interp (CaseInlInr t0 (x1,t1) (x2,t2)) = case (interp t0) of
    VInl v -> interp $ subst x1 t0 t1
    VInr v -> interp $ subst x2 t0 t2     
...
gamma |- (CaseInlInr t0 (x1,t1) (x2,t2)) = gamma |- t0 >>= \a ->
                              gamma |- (x1,t1) >>= \b ->
                              gamma |- (x2,t2) >>= \c ->
                              case a of
                                (TSum t11 t12) -> if t12 == b then Just t12 else Nothing
                                otherwise -> Nothing
