          ...
          --gamma |- (SumType x y)      = Just TSumType
          ...
          gamma |- (SumType t1 t2)    = gamma |- t1 >>= \inl ->
                                        gamma |- t2 >>= \inr ->
                                        case t1 of
                                          (TSumType t1 t2) -> if t1 == inl then Just t1 else Nothing
                                          otherwise -> Nothing
                                        --if(t1 == inl || t1 == inr) then return TSumType else if                                
